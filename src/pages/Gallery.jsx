...
function Gallery() {
    ...
    const { allPhotos, favorites, groupedPhotos, groupedFavorites } = useMemo(() => {
        ...
        // Sort by the date folder (e.g., "2026-01-02"), not by uploadedAt
        // This ensures photos appear in chronological order by their actual dates
        const getTime = (p) => DateTime.fromISO(p.date).toMillis();
    ...
    const handleStar = (photo) => {
        if (photo && photo.date !== undefined && photo.index !== undefined) {
            togglePhotoStar(photo.date, photo.index);
        }
    };
